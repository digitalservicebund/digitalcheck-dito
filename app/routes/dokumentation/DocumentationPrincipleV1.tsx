...
export function PrincipleWithExample({
  prinzip,
}: Readonly<{
  ...
  return (
    <div className="space-y-8">
      {simplifiedFlow ? (
        <Badge principleNumber={prinzip.Nummer}>Prinzip {prinzip.Nummer}</Badge>
      ) : (
...
export default function DocumentationPrincipleV1({
  currentUrl,
  nextUrl,
  previousUrl,
  prinzip,
        ...
        <form {...form.getFormProps()} className="space-y-40">
          ...
          {form.field("answer").value() === principlePages.radioOptions[1] && (
            <NegativeAnswerFormElements
              scope={form.scope("reasoning") as FormScope<string>}
            />
          )}
